...
void ComputePartitionInfo(params_t *params, graph_t *graph, idx_t *where)
{
  ...
  fprintf( fp, " - Edgecut: %"PRIDX", communication volume: %"PRIDX".\n\n",
  ...
  for (j=0; j<ncon; j++) {
    ...
    fprintf( fp, "     constraint #%"PRIDX":  %5.3"PRREAL" out of %5.3"PRREAL"\n",
        j, unbalance,
  ...
  if (ncon == 1) {
    ...
    fprintf( fp, " - Most overweight partition:\n"
  ...
  fprintf( fp, " - Subdomain connectivity: max: %"PRIDX", min: %"PRIDX", avg: %.2"PRREAL"\n\n",
  ...
  if (ncmps == nparts)
    fprintf( fp, " - Each partition is contiguous.\n");
  else {
    if (IsConnected(graph, 0)) {
      ...
      fprintf( fp, " - There are %"PRIDX" non-contiguous partitions.\n"
      ...
      fprintf( fp, " - The original graph had %"PRIDX" connected components and the resulting\n"
