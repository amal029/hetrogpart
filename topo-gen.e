...
k_arry_n_mesh(){
  ...
  for (;i<4;++i){
    for(;j<4;++j){
      ...
      if (i==0){
 if (j==0){
   MESH[i][j].EDGES[counter++] = (i*4 +j)+1;
   MESH[i][j].EDGES[counter++] = (i*4 +j)+1;
 }
 else if (j==4 -1){
 }
...
int
print(char *of, int counter){
  char file [256], file2[256];
  FILE *fd =((void *)0), *fd2 = ((void *)0);

  int i=0,j=0;
  for (;i<4;++i){
    for (;j<4;++j){

      fprintf(__stderrp,"%d:%d\n",i,j);
      int k=0;
      switch (counter){
      case 0:
 __builtin___snprintf_chk (file, 255, 0, __builtin_object_size (file, 2 > 1), "%s.%s",of,"MIPS.VEC.NUM");
 __builtin___snprintf_chk (file2, 255, 0, __builtin_object_size (file2, 2 > 1), "%s.%s",of,"MIPS.NUM.VEC");
 fd = fopen(file,"w");
 fd2 = fopen(file2,"w");
 fprintf(fd,"%d %d %s %d\n",(4*4), ((4*(4 -1))+(4*(4 -1))), "010", 3);
 fprintf(fd2,"%d %d %s %d\n",(4*4), ((4*(4 -1))+(4*(4 -1))), "010", 3);
 ...
 break;
      case 1:
 __builtin___snprintf_chk (file, 255, 0, __builtin_object_size (file, 2 > 1), "%s.%s",of,"VEC.MIPS.NUM");
 __builtin___snprintf_chk (file2, 255, 0, __builtin_object_size (file2, 2 > 1), "%s.%s",of,"VEC.NUM.MIPS");
 fd = fopen(file,"w");
 fd2 = fopen(file2,"w");
 fprintf(fd,"%d %d %s %d\n",(4*4), ((4*(4 -1))+(4*(4 -1))), "010", 3);
 fprintf(fd2,"%d %d %s %d\n",(4*4), ((4*(4 -1))+(4*(4 -1))), "010", 3);
 ...
 break;
      case 2:
 __builtin___snprintf_chk (file, 255, 0, __builtin_object_size (file, 2 > 1), "%s.%s",of,"NUM.VEC.MIPS");
 __builtin___snprintf_chk (file2, 255, 0, __builtin_object_size (file2, 2 > 1), "%s.%s",of,"NUM.MIPS.VEC");
 fd = fopen(file,"w");
 fd2 = fopen(file2,"w");
 fprintf(fd,"%d %d %s %d\n",(4*4), ((4*(4 -1))+(4*(4 -1))), "010", 3);
 fprintf(fd2,"%d %d %s %d\n",(4*4), ((4*(4 -1))+(4*(4 -1))), "010", 3);
 ...
 break;
      default: fprintf(__stderrp,"FUCKED!!\n"); exit(1);
      }
      for(;k<(2 -1)*4;++k){
...
main(int argc, char **argv){
  if (argc <= 1){
    fprintf(__stderrp,"Please run.... topo-gen <file-name>\n\n\n");
    exit(0);
  }
